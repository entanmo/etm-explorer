              ...
              <div class="list-area2 d-ib f-l">
                <p>
                  ...
                  <span
                    class="c-0283ff td-u"
                    @click="address"
                    :data-id="item.generatorId"
                  >{{item.generatorId}}</span>
                </p>
                <p>
                  {{$t('home_txns')}}:
                  <span class="c-0283ff">{{item.numberOfTransactions}}</span>
                </p>
              </div>
            ...
            <div class="list-box" v-for="(item,index) in transacList" :key="index">
              ...
              <div class="list-area1 d-ib f-l">
                <a-tooltip :trigger="['hover','click']">
                  ...
                  <p
                    class="ellipsis c-0283ff td-u"
                    @click="transacDetails"
                    :data-id="item.id"
                  >{{item.blockId}}</p>
                </a-tooltip>
  ...
  methods: {
    ...
    async _tokens() {
      try {
        ...
        this.marketCap =
          parseFloat(result.data.data.tokenCount) * parseFloat(this.last);
      } catch (err) {
        console.log(err);
      }
    },
    ...
    async _getBlocksList() {
      try {
        ...
        this.nowTime = parseInt(
          (new Date().getTime() - 1539316800000 - 8 * 60 * 60 * 1000) / 1000
        );
        this.blocksList = result.data.blocks.slice(0, 6);
        this.blockTime =
          (result.data.blocks[0].timestamp - result.data.blocks[99].timestamp) /
          99;
      } catch (err) {
        console.log(err);
      }
    ...
    blockDetails(e) {
      this.$router.push({
        path: "blockdetails",
        query: { blockId: e.srcElement.dataset.id }
      });
    },
    transacAll() {
    ...
    transacDetails(e) {
      this.$router.push({
        path: "transacdetails",
        query: { transacId: e.srcElement.dataset.id }
      });
    },
    address(e) {
      this.$router.push({
        path: "address",
        query: { addressId: e.srcElement.dataset.id }
      });
    }
  },
  components: {
...
.list-box .list-area3 img {
  width: 18px;
  height: 18px;
  margin: 3px 1px 0 0;
  vertical-align: top;
}
.list-box .list-area3 p {
  display: inline-block;
}
.details-transac .list-box .list-area1 {
  width: 82px;
}
...
@media screen and (max-width: 1050px) {
  ...
  .quick .quick-list:nth-last-child(1),
  .quick .quick-list:nth-last-child(2),
  .quick .quick-list:nth-last-child(3) {
    width: 32%;
  }
}
